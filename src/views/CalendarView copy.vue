<template>
  <div class="px-1 md:px-2 w-full mx-auto pt-1 md:pt-1 text-base-content">
    ...
    <ContentModal v-if="selectedContent" :content="selectedContent" />
  </div>
</template>
...
import { useAuthStore } from '@/stores'; // Import Pinia store
import ContentModal from '../components/ContentModal.vue';

// Inisialisasi Auth Store
const authStore = useAuthStore();
const isLoading = ref(true);
const contentData = ref([]); // Variabel JSON penampung utama
const selectedContent = ref(null);

async function showDetail(content) {
  selectedContent.value = content;
  // Menunggu Vue menyelesaikan update DOM setelah data selectedContent diisi
  await nextTick();
...
const contentData = ref([
  { id: 1, Tanggal_Rilis: new Date().toISOString(), Jam_Rilis: '09:00', Status: 'published', Rubrikasi: 'News', Judul_Cover: 'Peluncuran Fitur Baru' },
  { id: 2, Tanggal_Rilis: new Date().toISOString(), Jam_Rilis: '14:30', Status: 'on editing', Rubrikasi: 'Social', Judul_Cover: 'Konten Edukasi Mingguan' },
  { id: 3, Tanggal_Rilis: new Date(new Date().setDate(new Date().getDate() + 2)).toISOString(), Jam_Rilis: '10:00', Status: 'draft', Rubrikasi: 'Ads', Judul_Cover: 'Campaign Ramadhan' }
]);
...
/* Transisi halus untuk hover sel */
.transition-all {
